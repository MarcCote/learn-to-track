...
def build_argparser():
    ...
    dataset.add_argument('--resample-streamlines', action='store_true',
                         help='if specified, streamlines in a same batch will all have the same number of points. Default: keep original sizes.')
...
def main():
    ...
    hyperparams_to_exclude = ['max_epoch', 'force', 'name', 'view', 'lookahead']
    ...
    with Timer("Loading dataset", newline=True):
        ...
        batch_scheduler = TractographyBatchScheduler(trainset,
                                                     ...
                                                     resample_streamlines=args.resample_streamlines)
    ...
    with Timer("Building trainer"):
        ...
        if args.view:
            ...
            # Partial derivatives
            partial_grad1 = theano.grad(loss.loss, loss.L2_errors_per_time_step)
            partial_grad2 = theano.grad(loss.loss, loss.regression_outputs)
            partial_grad3 = theano.grad(loss.loss, loss.avg_L2_error_per_seq)
            min_abs_partial_grad1 = views.MonitorVariable(partial_grad1)
            min_abs_partial_grad2 = views.MonitorVariable(partial_grad2)
            min_abs_partial_grad3 = views.MonitorVariable(partial_grad3)

            min_abs_gradients = views.MonitorVariable(T.min(list(map(lambda d: abs(d).min(), loss.orig_gradients.values()))))
            clipped_gradients_norm = views.MonitorVariable(T.sqrt(sum(map(lambda d: T.sqr(d).sum(), loss.gradients.values()))))
            directions_norm = views.MonitorVariable(T.sqrt(sum(map(lambda d: T.sqr(d).sum(), optimizer.directions.values()))))
            ...
            # logger = tasks.Logger(min_abs_gradients, clipped_gradients_norm, directions_norm, loss_monitor, each_k_update=1)
            logger = tasks.Logger(min_abs_gradients, min_abs_partial_grad1, min_abs_partial_grad2, min_abs_partial_grad3, directions_norm, loss_monitor, each_k_update=1)
            ...
            def _plot_at_each_update(*args, **kwargs):
                ...
                plt.subplot(141)
                plt.plot(np.array(logger.get_variable_history(0)).flatten(), label="Min. abs grad")
                plt.legend()

                plt.subplot(142)
                plt.plot(np.array(logger.get_variable_history(1)).flatten(), label="Clipped ||g||")
                plt.legend()

                plt.subplot(143)
                plt.plot(np.array(logger.get_variable_history(2)).flatten(), label="||d||")
                plt.legend()

                plt.subplot(144)
                plt.plot(np.array(logger.get_variable_history(3)).flatten(), label="loss")
                plt.legend()

                plt.draw()

            trainer.append_task(tasks.Callback(_plot_at_each_update, each_k_update=1))
